...
\begin{itemize}
    ...
    \item Functors of the form $\lambda f_1...f_n.F$ are typed $\mc{C}^n \to \mc{D}$, with coherence for the associativity of products assumed to be known;
    \item Lambda abstraction and the left unitor use the same letter $\lambda$, but the first is used only in the form $\lambda x.y$, so the local meaning is clear;
    \item In general, typing is assumed to be clear from the context.
\end{itemize}
...
\begin{definition}[Monoidal category]
    A \emph{monoidal category (MC)} is a tuple
    ...
    \begin{itemize}
        ...
        \item[$\bullet$] $\alpha: \lambda fgh. f \otimes (g \otimes h) \xto{\simeq} \lambda fgh. (f \otimes g) \otimes h$ is an \emph{associator},
...
\begin{definition}[Strict monoidal category]
    A \emph{strict monoidal category} is an MC with $\alpha$, $\rho$ and $\lambda$ being identities. Thus,
    \begin{itemize}
...
\label{def:braided_mc}
    \emph{Braided monoidal category (BMC)} is an MC equipped with \emph{braiding} $$\sigma: \lambda fg. f \otimes g \to \lambda fg. g \otimes f$$ satisfying the \emph{hexagon} diagrams:
...
\begin{definition}[Closed monoidal category]
    A \emph{closed monoidal category} is an MC where for each $f: \arr(\mc{C})$ partial tensor $f \otimes -$ has the right adjoint $[f, -]$. The functor $$[f, -]: \mc{C}^{\op} \times \mc{C} \to \mc{C}$$ is called \emph{internal hom} (note, that $f$ can also be typed as $\arr(\mc{C})$).
\end{definition}
...
\begin{definition}[Monoidal natural transformation] Let $$(F, \phi), (G, \psi):(\mc{C}, \otimes, I_{\mc{C}}) \to (\mc{C}, \bullet, I_{\mc{D}})$$
    be monoidal. A \emph{monoidal natural transformation} $$\theta: (F, \phi) \to (G, \psi)$$ is a natural transformation $\theta: F \to G$ satisfying the diagrams
    % https://q.uiver.app/#q=
    \[\begin{tikzcd}
        {Fa \bullet Fb} & {Ga \bullet G b} && {I_{\mathcal{D}}} \\
        {F(a \otimes b)} & {G(a \otimes b)} & {FI_{\mathcal{D}}} && {GI_{\mathcal{D}}}
        \arrow["{\theta_a \bullet \theta_b}", from=1-1, to=1-2]
        \arrow["\phi"', from=1-1, to=2-1]
        \arrow["\psi", from=1-2, to=2-2]
        \arrow["{\phi_0}"', from=1-4, to=2-3]
        \arrow["{\psi_0}", from=1-4, to=2-5]
        \arrow["{\theta_{a \otimes b}}"', from=2-1, to=2-2]
        \arrow["{\theta_I}"', from=2-3, to=2-5]
    \end{tikzcd}\text{.}\]
...
\subsubsection{Coherence theorems}

\begin{theorem}[Mac Lane's Coherence Theorem]
    Every MC is monoidally equivalent to a strict monoidal category. Moreover, every diagram of canonical isomorphisms (built from $\alpha$, $\lambda$, $\rho$) commutes.
\end{theorem}

\begin{theorem}[Coherence for Symmetric Monoidal Categories]
    In a SMC, every diagram built from $\alpha$, $\lambda$, $\rho$, $\sigma$ (and their inverses) commutes.
\end{theorem}

\begin{theorem}[Coherence for Braided Monoidal Categories]
     In a BMC, all diagrams of canonical maps commute (Joyal-Street).
\end{theorem}

\begin{theorem}[Kelly-Mac Lane Coherence]
    For \emph{symmetric monoidal closed categories (SMCC)}, all diagrams of canonical maps (including those involving internal homs) commute.
\end{theorem}
