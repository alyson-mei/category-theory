...
\begin{example}[Exception monad]
    Consider a type $E$ of exceptions (error values) and a functor $\Exc: \mc{C} \to \mc{S}$, which sends a type to its coproduct with the exceptions type:
    ...
    \end{itemize}
    The diagrams (commutativity is obvious on $\mc{S}$et):
    % https://q.uiver.app/#q=
\[\begin{tikzcd}
	{A + E} && {A + E + E} & {A + E + E} && {A + E} \\
	&& {A + E} & {A + E}
	\arrow["{\eta_{A + E} = i_{1, A + E}}", from=1-1, to=1-3]
	\arrow["\id"', from=1-1, to=2-3]
	\arrow["{\nabla_{A + E, E}}", from=1-3, to=2-3]
	\arrow["{\id_{A} + \nabla_{E, E}}"', from=1-4, to=2-4]
	\arrow["{\eta_A + \id_E = i_{1, A} + \id_E}"', from=1-6, to=1-4]
	\arrow["\id", from=1-6, to=2-4]
\end{tikzcd}
\text{,}
\]
% https://q.uiver.app/#q=WzAsNCxbMCwwLCJBICszRSJdLFsyLDAsIkEgKzJFIl0sWzAsMiwiQSsyRSJdLFsyLDIsIkErRSJdLFswLDEsIlxcbmFibGFfe0EgKyBFfSArIFxcaWRfRSJdLFswLDIsIlxcbmFibGFfe0EgKyAyRSwgRX0iLDJdLFsyLDMsIlxcbmFibGFfe0ErRSwgRX0iLDJdLFsxLDMsIlxcbmFibGFfe0ErRSwgRX0iXV0=
\[\begin{tikzcd}
	{A +3E} && {A +2E} \\
	\\
	{A+2E} && {A+E}
	\arrow["{\nabla_{A + E} + \id_E}", from=1-1, to=1-3]
	\arrow["{\nabla_{A + 2E, E}}"', from=1-1, to=3-1]
...
\begin{example}[$\Kl(\List)$]
One of the possible interpretations of $f: A \to TB$ is a non-deterministic calculation: $a$ is mapped by $f$ to the list of all possible results. The composition corresponds to getting all possible results from sequential non-deterministic calculations:  
