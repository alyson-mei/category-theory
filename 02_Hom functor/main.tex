...
\begin{itemize}
    ...
    \item For brevity, when the context is clear, we assume
    \[
...
\begin{definition}[Set-valued Hom functor]
The set-valued Hom functor $$\lambda AB.\mc{C}(A,B): \mc{C}^\op \times \mathcal{C} \to \mc{S}et,$$
is defined as follows:
\begin{itemize}
    \item[$\bullet$] $\mc{C}(A, B)$ is the set of morphisms from $A$ to $B$,
    \item[$\bullet$] Its action on morphisms is given by
    \[
...
The functors
\[
    \mc{C}(A, -) := \lambda B.\,\mathcal{C}(A,B) : \mathcal{C} \to \mathcal{S}et
    \quad\text{and}\quad
    \mc{C}(-, B) := \lambda A.\,\mathcal{C}(A,B) : \mathcal{C}^{\op} \to \mathcal{S}et
\]
