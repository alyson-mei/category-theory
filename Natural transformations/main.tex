...
\begin{definition}[Natural transformations] Let $F, G: \mc{C} \to \mc{D}$. Natural transformation $\alpha$ between $F$ and $G$ is a collection of morphisms $(\alpha_c \ |\ {c: \obj{C}})$ satisfying the diagrams
...
Let $F, G, H: \mc{C} \to \mc{D}$ with natural transformations $\alpha: F \to G$ and $\beta: G \to H$. The vertical composition $\beta \circ \alpha$ of natural transformations $\beta$ and $\alpha$ is defined componentwise: 
...
\begin{definition}[Horizontal composition of natural transformations] Let $F_1, F_2: \mc{C} \to \mc{D}$, $G_1, G_2: \mc{D} \to \mc{E}$ with natural transformations $\alpha: F_1 \to F_2$ and $\beta: G_1 \to G_2$. The horizontal composition $\beta \bullet \alpha$ of natural transformations $\beta$ and $\alpha$ is defined as the transformation of images of $\alpha_c$ under the action of $F_1$ and $F_2$ by $\beta$:
...
\begin{definition}[Whiskering] Wiskering is defined as a horizontal composition with identity natural transformation:
...
\text{.}
\]
Then there is an interchange law between the natural transformations:
...
\section{Advanced whiskering}
The naturality diagram from Def 1.1. basically means, that for given $f: \arr(\mc{C})$
...
using some polymorphic notation? We already have the notation for whiskering, but so far we only defined it on $\obj(\mc{C})$.
